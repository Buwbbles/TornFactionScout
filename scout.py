...
DEFAULT_CONFIG = {
    ...
    "discovery": {
        "hof_pages": 10,
        "_comment_hof": "Each page = 100 factions from the respect hall of fame.",
...
def cmd_discover(conn, cfg, client):
    ...
    print("Hall of fame: %d pages" % pages)
...
def _id_sweep(conn, client, start, end):
    ...
    print("ID sweep from %d to %d (resumable)" % (cursor, end))
    hits = 0
    ...
    for i in range(0, len(todo), 200):
        ...
        for p in data or []:
            ...
            conn.execute("INSERT OR REPLACE INTO player_stat VALUES (?,?,?,?,?,?,?)", (
                ...
                p.get("source")))
        conn.commit()
        print("  %d/%d" % (min(i + 200, len(todo)), len(todo)))
        ...
        if mem:
            ...
            if len(est) >= 5:
                ...
                for k in ("stats_total", "stats_median", "stats_min", "stats_p10",
                          "stats_max", "stats_top10_avg", "wall_1b", "wall_10b", "your_percentile"):
                    rec[k] = None
...
def main():
    ...
    p.add_argument("command", choices=["init", "demo", "discover", "wars", "enrich",
                                       ...
                                       "prune", "history-save", "history-load"])
    ...
    conn = open_db()
    needs_key = args.command in ("discover", "wars", "enrich", "stats", "run", "snapshot")
    ...
    fn = {"demo": cmd_demo, "discover": cmd_discover, "wars": cmd_wars,
          ...
          "history-load": cmd_history_load}[args.command]
